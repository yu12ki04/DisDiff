...
class TimestepBlockShift(nn.Module):
    """
    Any module where forward() takes timestep embeddings as a second argument.
    """
    @abstractmethod
...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock, TimestepBlockShift):
    """
    ...
    def forward(self, x, emb, context=None):
        for layer in self:
            if isinstance(layer, ResBlockShift):
...
class ResBlockShift(TimestepBlock):
    """
    A residual block that can optionally change the number of channels.
    :param channels: the number of input channels.
    :param emb_channels: the number of timestep embedding channels.
    ...
    def forward(self, x, emb, cond_emb):
        """
        Apply the block to a Tensor, conditioned on a timestep embedding.
        :param x: an [N x C x ...] Tensor of features.
        :param emb: an [N x emb_channels] Tensor of timestep embeddings.
    ...
    def _forward(self, x, emb, cond_emb):
        if self.updown:
            in_rest, in_conv = self.in_layers[:-1], self.in_layers[-1]
            h = in_rest(x)
            h = self.h_upd(h)
        ...
        if self.use_scale_shift_norm:
            ...
            pre_h = out_norm(h)
            scale, shift = th.chunk(emb_out, 2, dim=1)
            h = pre_h * (1 + scale) + shift
            cond_scale, cond_shift = th.chunk(cond_emb_out, 2, dim=1)
            h = h * (1 + cond_scale) + cond_shift
            h = out_rest(h)
        else:
            h = h + emb_out + cond_emb_out
            h = self.out_layers(h)
        return self.skip_connection(x) + h
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.seprate_decoder = seprate_decoder
        self.wo_part_emb = wo_part_emb
        self.orth_emb = orth_emb
        self.repre_emb_channels = repre_emb_channels
        self.ckpt_path = ckpt_path
        self.load_ema_ckpt = load_ema_ckpt
        ...
        if self.seprate_decoder:# こっちが呼ばれる
            ...
            self.repre_embed = linear(self.repre_emb_channels*self.latent_unit, self.time_embed_dim)
    ...
    def forward(self, x, timesteps=None, context=None, y=None,**kwargs):
        ...
        with th.no_grad(): # 勾配計算をしない(事前学習済みのパラメータを使用)
        ...
        for ddx, idx in enumerate(range(self.latent_unit)):
            ...
            if ddx == 0:
                with th.no_grad():
                    h = self.middle_block(h0, emb, None)
                    jdx = 0
                    for module in self.output_blocks:
                        h = th.cat([h, hs[-jdx-1]], dim=1)
                        h = module(h, emb, None)
                        jdx += 1
                    h = h.type(x.dtype)
                    pred = self.out(h) 
    ...
    def init_model(self):
        ...
        try:
            ...
            print("load scale_factor failed, calculate it !")

        # model_keys = [k.replace("model.","") for k in state_dict.keys() if "model." in k]
        model_keys = [k.replace("model.","").replace("diffusion_","diffusion_model") for k in state_dict.keys() if "model." in k]
        keys_dic = dict([(k.replace(".",""),k) for k in model_keys])
        if not self.load_ema_ckpt:
            model_dict = {}
            for key in state_dict.keys():
                if "model_ema." in key and "decay" not in key and "num_updates" not in key:
                    model_dict[keys_dic[key.replace("model_ema.","")].replace("diffusion_model","")] = state_dict[key]
        else:
            model_dict = {}
            for key in state_dict.keys():
                if "model." in key:
                    model_dict[key.replace("model.","").replace("diffusion_","")] = state_dict[key]
        m, u = self.load_state_dict(model_dict, strict=False)
        if len(u) > 0:
            print("unexpected keys:")
            print([ui for ui in u if "first_stage" not in ui])
...
def torch_log2(x):
    return th.log(x) / np.log(2.0)


def torch_pade13(A):
    b = th.tensor([64764752532480000., 32382376266240000., 7771770303897600.,
                      1187353796428800., 129060195264000., 10559470521600.,
                      670442572800., 33522128640., 1323241920., 40840800.,
                      960960., 16380., 182., 1.], dtype=A.dtype, device=A.device)

    ident = th.eye(A.shape[1], dtype=A.dtype).to(A.device)
    A2 = th.matmul(A, A)
    A4 = th.matmul(A2, A2)
    A6 = th.matmul(A4, A2)
    U = th.matmul(A,
                     th.matmul(A6, b[13] * A6 + b[11] * A4 + b[9] * A2) + b[7] * A6 + b[5] * A4 +
                     b[3] * A2 + b[1] * ident)
    V = th.matmul(A6, b[12] * A6 + b[10] * A4 + b[8] * A2) + b[6] * A6 + b[4] * A4 + b[2] * A2 +\
        b[0] * ident
    return U, V
def torch_expm(A):
    n_A = A.shape[0]
    A_fro = th.sqrt(A.abs().pow(2).sum(dim=(1, 2), keepdim=True))

    # Scaling step
    maxnorm = th.tensor([5.371920351148152], dtype=A.dtype, device=A.device)
    zero = th.tensor([0.0], dtype=A.dtype, device=A.device)
    n_squarings = th.max(zero, th.ceil(torch_log2(A_fro / maxnorm)))
    A_scaled = A / 2.0 ** n_squarings
    n_squarings = n_squarings.flatten().type(th.int64)

    # Pade 13 approximation
    U, V = torch_pade13(A_scaled)
    P = U + V
    Q = -U + V
    R = th.linalg.solve(P, Q)
    ...
    R = th.stack(res)
    expmA = R[n_squarings, th.arange(n_A)]
    return expmA[0]