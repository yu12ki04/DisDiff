...
class DataModuleFromConfig(pl.LightningDataModule):
    ...
    def _train_dataloader(self):
        return DataLoader(self.datasets["train"], batch_size=self.batch_size,
                          num_workers=self.num_workers, shuffle=True, collate_fn=self.custom_collate_fn)
    ...
    def _val_dataloader(self, shuffle=False):
        return DataLoader(self.datasets["validation"],
                          ...
                          shuffle=shuffle, collate_fn=self.custom_collate_fn)

    def _test_dataloader(self, shuffle=False):
        return DataLoader(self.datasets["test"], batch_size=self.batch_size,
                          num_workers=self.num_workers, worker_init_fn=self._worker_init_fn,
                          shuffle=shuffle, collate_fn=self.custom_collate_fn)
    ...
    def custom_collate_fn(self, batch):
        # バッチ全体からimageとtextをそれぞれ抽出
        images = [item['image'] for item in batch]
        texts = [item['text'] for item in batch]  # ここで各サンプルのリストをそのまま保持

        # Tensorに変換する場合、画像のみTensorに変換
        images = torch.stack(images)  # 画像をテンソルに変換してバッチ化

        return {'image': images, 'text': texts}  # テキストはリストのまま返す
# class DataModuleFromConfig(pl.LightningDataModule):
#     def __init__(self, batch_size, train=None, validation=None, test=None, predict=None,
#                  wrap=False, num_workers=None, shuffle_test_loader=False, use_worker_init_fn=False,
#                  shuffle_val_dataloader=False):
